...
class Install {
	...
	public function add_table_on_create_blog( $blog_id ) {
		if ( is_plugin_active_for_network( 'sms-for-woocommerce/sms-for-woocommerce.php' ) ) {
			switch_to_blog( $blog_id );
	...
	public function remove_table_on_delete_blog( $tables ) {
		...
		foreach ( array( 'wpsmstowoo_subscribes', 'wpsmstowoo_subscribes_group', 'wpsmstowoo_send' ) as $tbl ) {
