...
class SFW_SMS_TO_Integrations {
    ...
    public function __construct() {
        ...
        if (isset($this->options['wc_notify_customer_enable'])) {
            add_action('woocommerce_new_order', array($this, 'wc_new_order_customer_notification'));
        }
        
        if (isset($this->options['wc_notify_order_enable'])) {
            add_action('woocommerce_new_order', array($this, 'wc_new_order_receipient_notification'));
        }        
            ...
            if ($this->options['wc_mobile_phone_field'] == 'customer_mobile_phone') {
                ...
                if (isset($c_user_id) && ($c_user_id != 0 ))
                {
                  ...
                  if (isset($_REQUEST['billing_phone'])) {
                    $mobile_phone = sanitize_text_field($_REQUEST['billing_phone']);
                  } else
                  if (isset($_REQUEST['_billing_phone'])) {
                    $mobile_phone = sanitize_text_field($_REQUEST['_billing_phone']);
                  }
            } else {
                $mobile_phone = '';
            }
    ...
    /**
     * Retrieve the users phone numbers of a subscription group
     * 
     * @author Christodoulou Panikos
     * @email christodoulou.panicos@cytanet.com.cy
     * @param subscription group ID
     * @return Array
     */
    private function getSubscriberGroupPhoneNumbers($subscribers_group_id) {

        global $wpdb;
        $status = 1;

        $just_mobile_phone_numbers = array();
        if ($subscribers_group_id != 0) {
            $mobile_phone_numbers = $wpdb->get_results(
                    $wpdb->prepare(
                            "SELECT mobile_phone FROM $wpdb->prefix" . "wpsmstowoo_subscribes WHERE status = %s
				 AND group_ID = %d",
                            $status, $subscribers_group_id
                    )
            );
        } else {
            $mobile_phone_numbers = $wpdb->get_results(
        ...
        foreach ($mobile_phone_numbers as $mobile_phone_number) {
            $just_mobile_phone_numbers [] = $mobile_phone_number->mobile_phone;
        }
        return $just_mobile_phone_numbers;
    }
