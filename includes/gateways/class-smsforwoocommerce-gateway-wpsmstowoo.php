...
class wpsmstowoo extends \SFW_SMS_TO_WOO\SFW_SMS_TO_Gateway {
    ...
    public function SendSMS() {        
        ...
        if ($no_of_characters>480) {
            return new \WP_Error('account-credit', __('You have exceeded the max limit of 480 characters', 'sms-for-woocommerce'));
        } 
   ...
   if ($bodyContent) {
        ...
        $headers = array(
            ...
            'X-Smsto-Integration-Name'  => 'smsforwoocommerce'
        );        
    ...
    public function GetCredit() {
        ...
        $response = wp_remote_get(\SFW_SMS_TO_WOO\Gateway\wpsmstowoo::getTariff() . '/api/balance?api_key=' . $this->api_key);
        if (!is_wp_error($response) && 200 == wp_remote_retrieve_response_code($response)) {
            $body = json_decode(wp_remote_retrieve_body($response));
