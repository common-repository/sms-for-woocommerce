...
class SFW_SMS_TO_WOO {
	...
	public function __construct() {
		...
		register_activation_hook( SFW_SMS_TO_WOO_DIR . 'sms-for-woocommerce.php', array( '\SFW_SMS_TO_WOO\Install', 'install' ) );
	...
	/**
	 * Redirect to specific tab so that it always contains the a response
	 * If the page =sms-for-woocommerce-outbox then add parameter response=message_id which is the default
	 * 
	 */    
        
       public function redirect_to_samepage_with_added_parameter() {
        if (substr($_SERVER["REQUEST_URI"], -31) == 'page=sms-for-woocommerce-outbox') {
    ...
    public function sfw_sms_to_woo_update_db() {
        ...
        if ($installer_wpsms_ver < SFW_SMS_TO_WOO_VERSION) {
            ...
            $table_name = $wpdb->prefix . 'smstowoo_send';
