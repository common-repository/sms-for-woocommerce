...
class SFW_SMS_TO_Outbox_List_Table extends \WP_List_Table {
	...
	function __construct() {
        ...
        function get_views(){
           ...
           $messages_url = admin_url('admin.php?page=sms-for-woocommerce-outbox&response=message_id');
           $class = ($current == 'message_id' ? ' class="current"' :'');
           $views['response_message'] = "<a href='{$messages_url}' {$class} >Messages</a>";

      
           $campaigns_url = admin_url('admin.php?page=sms-for-woocommerce-outbox&response=campaign_id');
	...
	function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'recipient':
				...
				$html = '<details>
						  <summary>' . __( $item[ $column_name ], 'sms-for-woocommerce' ) . '</summary>
    ...
    function column__id($item ) {
		...
		$actions = array(
			'resend' => sprintf( '<a href="?page=%s&action=%s&response=%s&ID=%s">' . __( 'Resend', 'sms-for-woocommerce' ) . '</a>', sanitize_text_field($_REQUEST['page']), 'resend', $request_response, $item['ID'] ),
			'delete' => sprintf( '<a href="?page=%s&action=%s&response=%s&ID=%s">' . __( 'Delete', 'sms-for-woocommerce' ) . '</a>', sanitize_text_field($_REQUEST['page']), 'delete', $request_response, $item['ID'] ),
                );
	...
	function get_columns() {
        ...
        $columns = array(
            ...
            '_id' => __('Campaign Id', 'sms-for-woocommerce'),
            'sender' => __('Sender', 'sms-for-woocommerce'),
            'type' => __('Type', 'sms-for-woocommerce'),
            'cost' => __('Cost', 'sms-for-woocommerce'),   
            'recipient' => __('Recipient', 'sms-for-woocommerce'),
            'message_count' => __('Total Messages', 'sms-for-woocommerce'),
            'sms_parts' => __('Parts', 'sms-for-woocommerce'),
            'sent' => __('Sent', 'sms-for-woocommerce'),
            'failed' => __('Failed', 'sms-for-woocommerce'),
            'pending' => __('Pending', 'sms-for-woocommerce'),       
	...
	function process_bulk_action() {
		...
		// Search action
        if (isset($_GET['s'])) {
            $prepare = $this->db->prepare("SELECT * from `{$this->tb_prefix}smstowoo_send` WHERE substring(response, 1, 5) = %s and (message LIKE %s OR recipient LIKE %s) ORDER BY date DESC ", $this->db->esc_like($search_response), '%' . $this->db->esc_like($_GET['s']) . '%', '%' . $this->db->esc_like($_GET['s']) . '%');
            $this->data = $this->get_data($prepare);
            $this->count = $this->get_total($prepare);
        } else {
            $prepare = $this->db->prepare("SELECT * from `{$this->tb_prefix}smstowoo_send` WHERE substring(response, 1, 5) = %s ORDER BY date DESC ", $this->db->esc_like($search_response));
            $this->data = $this->get_data($prepare);
            $this->count = $this->get_total($prepare);
        }
		...
		if ( 'bulk_delete' == $this->current_action() ) {
			...
			foreach ( $ids as $id ) {
				$this->db->delete( $this->tb_prefix . "smstowoo_send", array( 'ID' => $id ) );
			}
			$this->data  = $this->get_data();
			$this->count = $this->get_total();
		...
		if ( 'delete' == $this->current_action() ) {
			$this->db->delete( $this->tb_prefix . "smstowoo_send", array( 'ID' => sanitize_text_field($_GET['ID']) ) );
			$this->data  = $this->get_data();
			$this->count = $this->get_total();
		...
		if ( 'resend' == $this->current_action() ) {
			global $wpsmstowoo;
			$error    = null;
			$result   = $this->db->get_row( $this->db->prepare( "SELECT * from `{$this->tb_prefix}smstowoo_send` WHERE ID =%s;", sanitize_text_field($_GET['ID'] )) );
			...
			$this->count = $this->get_total();
                        ...
                        if (isset($_REQUEST['response'])){
                            if (sanitize_text_field($_REQUEST['response']) == 'message_id') {
                                $url = admin_url( 'admin.php?page=sms-for-woocommerce-outbox&response=message_id' );
                            } 
                            if (sanitize_text_field($_REQUEST['response']) == 'campaign_id') {
                                $url = admin_url( 'admin.php?page=sms-for-woocommerce-outbox&response=campaign_id' );
                            }
                        } else
                        {
                            $url = admin_url( 'admin.php?page=sms-for-woocommerce-outbox&response=message_id' );
    ...
    function get_data($query = '') {
        ...
        if (!$query) {
            $query = 'SELECT * FROM `' . $this->tb_prefix . 'smstowoo_send` WHERE substring(`response`, 1, 5) = ' . $search_response . ' ORDER BY date DESC LIMIT ' . $this->limit . ' OFFSET ' . $page_number;
        } else {
	...
	function get_total( $query = '' ) {
            ...
            if (!$query) {
                $query = 'SELECT * FROM `' . $this->tb_prefix . 'smstowoo_send` WHERE substring(`response`, 1, 5) = ' . $search_response. ' ORDER BY date DESC ';
            }
            $result = $this->db->get_results($query, ARRAY_A);
            $result = count($result);
