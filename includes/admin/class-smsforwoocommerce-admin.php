...
class SFW_SMS_TO_Admin {
	...
	public function admin_menu() {
		$hook_suffix = array();
		add_menu_page( __( 'SMS for WooCommerce - by SMS.to', 'sms-for-woocommerce' ), __( 'WooCommerce SMS', 'sms-for-woocommerce' ), 'wpsmstowoo', 'sms-for-woocommerce', array( $this, 'outbox_callback' ), 'dashicons-smartphone' );
	...
	public function privacy_assets() {
		...
		wp_enqueue_script( 'postbox' );
                ...
                wp_register_script( 'wpsmstowoo-privacy', SFW_SMS_TO_WOO_URL . 'assets/js/privacy.js', true, SFW_SMS_TO_WOO_VERSION );
                wp_enqueue_script( 'wpsmstowoo-privacy');                

		add_meta_box( 'privacy-meta-1', esc_html( get_admin_page_title() ), array( SFW_SMS_TO_Privacy::class, 'privacy_meta_html_gdpr' ), $pagehook, 'side', 'core' );
		add_meta_box( 'privacy-meta-2', __( 'Export User’s Data related to SMS for WooCommerce - by SMS.to', 'sms-for-woocommerce' ), array( SFW_SMS_TO_Privacy::class, 'privacy_meta_html_export' ), $pagehook, 'normal', 'core' );
		add_meta_box( 'privacy-meta-3', __( 'Erase User’s Data related to SMS for WooCommerce - by SMS.to', 'sms-for-woocommerce' ), array( SFW_SMS_TO_Privacy::class, 'privacy_meta_html_delete' ), $pagehook, 'normal', 'core' );
	}
	...
	public function meta_links( $links, $file ) {
		if ( $file == 'sms-for-woocommerce/sms-for-woocommerce.php' ) {
			$rate_url = 'http://wordpress.org/support/view/plugin-reviews/sms-for-woocommerce?rate=5#postform';
			$links[]  = '<a href="' . $rate_url . '" target="_blank" class="wpsmstowoo-plugin-meta-link" title="' . __( 'Click here to rate and review this plugin on WordPress.org', 'sms-for-woocommerce' ) . '">' . __( 'Rate this plugin', 'sms-for-woocommerce' ) . '</a>';
